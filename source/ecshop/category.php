...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0  AND g.goods_number > 0 ".
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id  AND g.goods_number > 0 AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
                       " WHERE ($children OR " . get_extension_goods($children) . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  AND g.goods_number > 0 '.
                       " AND a.attr_id='$value' ".
